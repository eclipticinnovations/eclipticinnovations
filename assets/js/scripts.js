/*-----------------------------------------------------------------------------------
    ...
    Theme Name: Ecliptic Innovations
    Theme URI: http://
    Description: Creative Agency & Portfolio
    Author: UI-ThemeZ
    Author URI: http://eclipticinnovations.in/
    Version: 1.0
...
$(function () {
    ...
    $(function () {
        ...
        function toggleRootClass() {
            const body = document.querySelector("body:not(.notmode)");
...
$(window).on("load", function () {
    ...
    if ($contactForm.length) {
        ...
        $contactForm.on('submit', function (e) {
            ...
            var originalText = $button.find('.text-uppercase').first().text() || $button.text();

            $button.prop('disabled', true).addClass('is-loading');
            if ($button.find('.text-uppercase').length) {
                $button.find('.text-uppercase').first().text('Sending...');
            }
            ...
            $.ajax({
                ...
                success: function (data) {
                    var type = data && data.type === 'success' ? 'success' : 'danger';
                    var messageText = data && data.message ? data.message : 'Thanks. We received your message and will get back to you shortly.';
                    var alertBox = '<div class="alert alert-' + type + '" role="alert">' + messageText + '</div>';
                ...
                error: function () {
                    $messages.html('<div class="alert alert-warning" role="alert">We could not submit the form automatically. Please email contact.eclipticinnovations@gmail.com or call +91 78690 13506.</div>');
                },
                complete: function () {
                    $button.prop('disabled', false).removeClass('is-loading');
                    if ($button.find('.text-uppercase').length) {
                        $button.find('.text-uppercase').first().text(originalText);
                    }
                }
            });
    ...
    $('.newsletter-form').on('submit', function (e) {
        ...
        var $form = $(form);
        var $button = $form.find('button[type="submit"]');
        var $feedback = $form.next('.newsletter-feedback');

        if (!$feedback.length) {
            $feedback = $('<p class="newsletter-feedback fs-14 mt-15px" role="status"></p>').insertAfter($form);
        }
        ...
        $.ajax({
            type: $form.attr('method') || 'POST',
            url: $form.attr('action') || 'newsletter.php',
            data: $form.serialize(),
            dataType: 'json',
            success: function (data) {
                var ok = data && data.type === 'success';
                $feedback.toggleClass('text-success', ok).toggleClass('text-warning', !ok)
                    .text((data && data.message) || 'Thanks for subscribing.');
                if (ok) form.reset();
            },
            error: function () {
                $feedback.removeClass('text-success').addClass('text-warning')
                    .text('Subscription could not be completed right now. Please try again later.');
            },
            complete: function () {
                $button.prop('disabled', false).removeClass('is-loading');
            }
